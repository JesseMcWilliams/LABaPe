# Common `network` module interface (DESIGN.md §6.2) — same variables
# as modules/network/libvirt, this is the Hyper-V implementation.

variable "environment_name" {
  description = "Name of the environment instance (matches the OpenTofu workspace, DESIGN.md §6.3)."
  type        = string
}

variable "mode" {
  description = "bridged (default) or nat — DESIGN.md §14. Only \"bridged\" is implemented as of M2; NAT (Internal switch + New-NetNat) lands in M7 (docs/networking.md §2)."
  type        = string
  default     = "bridged"
  ...
  validation {
    ...
    error_message = "mode = \"nat\" is not implemented yet for the Hyper-V backend (docs/networking.md §2 documents the design; it's scheduled for M7). Use \"bridged\" for now."
  }
}
...
variable "physical_nic" {
  description = <<-EOT
    ...
    External switch to (e.g. "Ethernet") — DESIGN.md §14. Only used when
...
# eventual NAT path (DESIGN.md §6.2), but unused while mode = "bridged".
