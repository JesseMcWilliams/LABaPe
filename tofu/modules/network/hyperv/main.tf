# Bridged mode (default, DESIGN.md §14): an External Hyper-V switch
# bound to the host's physical NIC. Unlike libvirt's bridge_device (a
# pre-existing host prerequisite, docs/networking.md §1), Hyper-V has no
...
# per DESIGN.md §17.4 — Hyper-V avoids it for free here).
#
# NAT mode (opt-in, M7) is where this module will manage an Internal
# switch + New-NetNat instead (docs/networking.md §2) — the `mode`
...
resource "hyperv_network_switch" "this" {
  ...
  notes                = "Managed by LABaPe (tofu/modules/network/hyperv) - DESIGN.md section 14."
