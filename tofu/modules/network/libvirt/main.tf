# Bridged mode (default, DESIGN.md §14) needs no libvirt resource at
...
# `network_id`, per the common network-module contract (DESIGN.md §6.2).
#
# The bridge itself (`nmcli connection add type bridge ...`) is a
# one-time host prerequisite documented in docs/networking.md §1, not
...
# `libvirt_network` resource (docs/networking.md §2) — the `mode`
