# Common `vm` module interface (Claude_Docs/Design_System-Overview.md §6.1). Both backends' vm
...
# backend module gets invoked does (tofu/backends/*, Claude_Docs/Design_System-Overview.md §6.3).
...
variable "os" {
  description = "Key into var.os_catalog, e.g. \"rocky9\". M1 ships only that one entry — Claude_Docs/Design_System-Overview.md §5's full OS matrix lands across M2-M6."
  type        = string
}
...
variable "network_id" {
  description = "Output of the network module (../network/libvirt) — the bridge device name in bridged mode (Claude_Docs/Design_System-Overview.md §6.2)."
  type        = string
}
...
variable "admin_credential" {
  description = "Bootstrap credential from Claude_Docs/Reference_Credentials.md. Only ssh_public_key is used on this (Linux-only, M1) backend path."
  type = object({
...
variable "template_vars" {
  description = "Extra values rendered into the answer file at provisioning time — e.g. management_source for firewall scoping (Claude_Docs/Reference_Credentials.md §7). Deliberately NOT where domain_name/dns_forward_ip go (Claude_Docs/Design_System-Overview.md §6.1's output table note)."
  type        = map(string)
  default     = {}
}
...
variable "libvirt_uri" {
  description = "The same connection URI the libvirt provider itself is configured with (Claude_Docs/Reference_Credentials.md §3) — needed again here because the iso_direct path shells out to virt-install via a local-exec provisioner, which runs outside the provider's own resource management and can't introspect its connection config."
  type        = string
}
...
variable "os_catalog" {
  description = <<-EOT
    ...
    (Claude_Docs/Design_System-Overview.md §5) is still out of scope; only what's actually
...
variable "vm_storage_path" {
  description = "Directory on the libvirt host where this VM's disk (and, for Windows, the generated answer-file ISO) are created. Claude_Docs/Design_Base-Images.md — kept off the small root filesystem by convention."
  type        = string
}
