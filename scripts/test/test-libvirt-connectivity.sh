#!/usr/bin/env bash
# docs/validate-setup.md §4 — confirms virsh/virt-install are present
...
if [ ! -f "$VAULT_FILE" ] || [ ! -f "$VAULT_PASS_FILE" ]; then
  echo "labape: libvirt — $VAULT_FILE or $VAULT_PASS_FILE not set up yet, skipping (docs/credentials.md §1)." >&2
  exit 0
fi

for cmd in virsh virt-install; do
  if ! command -v "$cmd" >/dev/null 2>&1; then
    echo "FAIL: $cmd not found — docs/install-opentofu.md §4 (sudo apt-get install -y libvirt-clients virtinst)." >&2
    exit 1
  fi
done
...
if virsh --connect "$libvirt_uri" list --all; then
  ...
  echo "FAIL: could not connect to $libvirt_uri — docs/install-opentofu.md §6 (check ssh-copy-id was run, libvirtd is running on the host, and the URI itself is correct)." >&2
  exit 1
fi
